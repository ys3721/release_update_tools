...
server_count=`ssh -o ConnectTimeout=1 $ip 'ls -lh / | egrep "data[0-1|5-6]$" | wc -l'`
process_count=`ssh -o ConnectTimeout=1 $ip 'ps -elf | grep GameServer | grep -v grep | wc -l'`

for port in `seq 3306 3309`
do
  merged=`ssh -o ConnectTimeout=1 $ip "/usr/local/mysql/bin/mysql -uroot -p123456 -h127.0.0.1 -P$port -N  wg_lj -e'select battleFieldVersion from t_sys_info'"`
...
if [[ "$server_count" != "$process_count" ]] && [ $server_count -ne $process_count ]
  then
    echo "$1所在的服务器好像是有机器挂了"
    python /data3/remote_execute/scripts/monitor/sms.py 15811302052 "$1所在的服务器好像是有机器挂了"
fi