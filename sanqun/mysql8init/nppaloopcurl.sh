...
function asyncSend() {
    for ((i=1;i<=$1;i++)); do
      echo "REQUEST=> http://dunk.qyz.feidou.com/sdk/sdk.login.go?${result_str}"
      curl --header "Connection: keep-alive" -d '' "http://dunk.qyz.feidou.com/sdk/sdk.login.go?${result_str}" &
    done
}
